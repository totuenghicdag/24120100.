...
void removeDuplicate(List*& L) {
    ...
    while (cur != nullptr) {
        NODE* prev = cur;
        NODE* check = cur->p_next;
        while (check != nullptr) {
            if (cur->key == check->key) {
                prev->p_next = check->p_next;
                delete check;
                check = prev->p_next;
            }
            else {
